#!/usr/bin/env python3
"""
AbangJal / Arijal Meutuwah — Auto-Ingest Bridge
...
def parse_caption_links(caption: str):
    ...
    title = clean_text.split("\n")[0].strip() if clean_text else "Produk Arijal Meutuwah"
    desc = clean_text if clean_text else "Review & Rekomendasi Gadget / Lifestyle Arijal Meutuwah"
...
def process_inbox_items():
    ...
    print(f"[+] Found {len(items)} NEW items in AbangJal Inbox. Initializing Google Workspace...")
    ...
    for item in items:
        ...
        product_id = f"ABANGJAL_{now_str}_{item_id}"
