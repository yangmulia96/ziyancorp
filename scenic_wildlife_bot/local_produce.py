#!/usr/bin/env python3
"""
Produksi 3 Video Short - render lokal pakai ffmpeg + Edge-TTS
"""
...
async def generate_tts(text, output_mp3):
    try:
        import edge_tts
        comm = edge_tts.Communicate(text, "id-ID-Gusti-RahayuNeural")
...
def create_video(search_keyword, audio_mp3, output_mp4):
    try:
        import requests
...
async def main():
    print("=" * 60)
    print("3 VIDEO SHORT - LOCAL RENDER")
    print("=" * 60)
    ...
    for i in range(1, 4):
        ...
        if m:
            ...
            print(f"   Hook: {m['hook_sentence'][:60]}...")
    ...
    for i, m in metas:
        ...
        ok = await generate_tts(m["hook_sentence"], audio)
        if not ok:
            print(f"  TTS failed")
            continue
        
        mp4 = f"short_{i}.mp4"
        video = create_video(m["search_keyword"], audio, mp4)
        if video:
            print(f"  OK: {video}")
