"""
Colab Production Engine - Renders viral wildlife video on Google Colab GPU
Injects full production pipeline as a Colab cell and executes it.
"""
import subprocess
import sys
import json
...
def build_colab_cell(data: dict) -> str:
    data_json = json.dumps(data, ensure_ascii=False)
    cell_code = COLAB_CELL_TEMPLATE.replace("{DATA_JSON}", data_json)
    return cell_code
...
if __name__ == "__main__":
    ...
    sample = {
        ...
        "hook_sentence": "This animal has never slept in its entire life.",
        "script_lines": ["Line 1", "Line 2", "Line 3", "Line 4", "Line 5", "Line 6", "Line 7"],
        "search_keyword": "ocean wildlife underwater",
    }
    code = build_colab_cell(sample)
    print(code[:500])
