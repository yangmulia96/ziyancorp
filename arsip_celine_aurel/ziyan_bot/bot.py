...
class ArchiveBot:
    ...
    async def on_media(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        chat_id = update.effective_chat.id
        if self.db.message_seen(chat_id, message.message_id):
            logger.info("Ignoring already processed message chat=%s msg=%s", chat_id, message.message_id)
            return
        session = self.session(chat_id)
        ...
        self.db.mark_message(chat_id, message.message_id)
